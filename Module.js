...
// PILOT Extension: Пассажирские перевозки (ДЕМО-ВЕРСИЯ, Pattern C: Existing Map)
// Использует существующую карту PILOT, что гарантирует работу кликов и отрисовки.
...
Ext.define('Store.passenger_transit.Module', {
    ...
    initModule: function () {
        var me = this;
        Ext.log('passenger_transit: initModule started');
        ...
        // Основная панель с прозрачным центром, чтобы карта PILOT была видна и кликабельна
        var mainPanel = Ext.create('Ext.panel.Panel', {
            ...
            items: [
                { 
                    region: 'center', 
                    html: '', 
                    bodyStyle: 'background: transparent; border: none; pointer-events: none;' 
                },
                Ext.create('Store.passenger_transit.view.RouteMemoPanel', { 
                    region: 'east', module: me, width: 300, split: true, collapsible: true, title: me.L('Мнемосхема'),
                    listeners: { afterrender: function() { me.memoPanel = this; } }
                }),
                Ext.create('Store.passenger_transit.view.TimelinePanel', { 
                    region: 'south', module: me, height: 180, split: true, title: me.L('График рейсов'),
        ...
        if (window.skeleton && skeleton.navigation && skeleton.mapframe) {
            ...
            if (skeleton.header && skeleton.header.insert) {
                skeleton.header.insert(6, {
                    ...
                    iconCls: 'fa fa-route',
                    tooltip: me.L('Пассажирские перевозки'),
            ...
            me.loadRoutes();
            // Автоматически выбираем первый маршрут для демонстрации
    ...
    // ==================== MAP HELPERS (Pattern C) ====================
    getPilotMap: function () {
        if (window.getActiveTabMapContainer) {
            return getActiveTabMapContainer();
        }
        return window.mapContainer || window.historyMapContainer || null;
    },

    // ==================== ДЕМО-МЕТОДЫ ====================
    loadRoutes: function () {
        Ext.log('passenger_transit: loadRoutes called');
    ...
    createRoute: function (name) {
        ...
        Ext.toast({ html: me.L('Маршрут создан (Демо)'), align: 't', timeout: 2000 });
    },
    ...
    addStop: function (routeId, stop) {
        ...
        if (route) {
            ...
            Ext.toast({ html: me.L('Остановка добавлена (Демо)'), align: 't', timeout: 2000 });
        }
    },
    ...
    saveRoutePoints: function (routeId, points, direction) {
        ...
        if (route) {
            ...
            me.selectRoute(routeId);
            Ext.toast({ html: me.L('Маршрут сохранен (Демо)'), align: 't', timeout: 2000 });
        }
    },
    ...
    // ==================== DRAWING ON EXISTING MAP ====================
    drawRoute: function (routeId, forwardPoints, backwardPoints) {
        var mapObj = this.getPilotMap();
        if (!mapObj || !mapObj.map) {
            Ext.log('passenger_transit: Map object not ready');
            return;
        }
        var leafletMap = mapObj.map;
    ...
    drawStops: function (routeId, stops) {
        ...
        stops.forEach(function (stop, index) {
            ...
            marker.bindPopup('<b>' + Ext.String.htmlEncode(stop.name) + '</b><br/>' + me.L('Остановка') + ' #' + (index + 1));
    ...
    selectRoute: function (routeId) {
        var me = this;
        Ext.log('passenger_transit: selectRoute', routeId);
    ...
    selectVehicle: function (vehicleId, routeId) {
        ...
        Ext.Msg.alert(me.L('ТС'), me.L('Выполнено рейсов') + ': <b>' + Math.floor(Math.random() * 5 + 3) + '</b><br/>(Демо-данные)');
    },
    ...
    enableRouteEditMode: function (routeId, direction) {
        ...
        var mapObj = me.getPilotMap();
        if (!mapObj || !mapObj.map) {
            Ext.Msg.alert(me.L('Ошибка'), me.L('Карта не найдена. Убедитесь, что вы находитесь в разделе Online или History.'));
            return;
        }
        var leafletMap = mapObj.map;
        ...
        Ext.toast({ html: me.L('Кликайте по карте для добавления точек. Правый клик - завершить.'), align: 't', timeout: 5000 });
    },
    ...
    drawEditingPolyline: function () {
        ...
        points.forEach(function (p, index) {
            ...
            marker.bindPopup(me.L('Точка') + ' #' + (index + 1));
    ...
    finishRouteEditing: function () {
        ...
        if (points.length < 2) {
            Ext.Msg.alert(me.L('Ошибка'), me.L('Минимум 2 точки'));
            return;
        }
        Ext.Msg.confirm(me.L('Сохранение'), me.L('Точек: ') + points.length + '. ' + me.L('Сохранить?'), function (btn) {
    ...
    disableRouteEditMode: function () {
        ...
        if (me.state.mapLayers.editingPolyline && mapObj && mapObj.map) {
            mapObj.map.removeLayer(me.state.mapLayers.editingPolyline);
            me.state.mapLayers.editingPolyline = null;
        }
        if (me.state.mapLayers.editingPoints && mapObj && mapObj.map) {
            me.state.mapLayers.editingPoints.forEach(function (m) { mapObj.map.removeLayer(m); });
            me.state.mapLayers.editingPoints = [];
        }
    ...
    showRouteEditToolbar: function () {
        var me = this;
        // Ищем mainPanel через skeleton
        var mainPanel = (skeleton.mapframe && skeleton.mapframe.items) ? skeleton.mapframe.items.getAt(0) : null;
        if (!mainPanel) return;
        ...
        if (!me.editToolbar) {
            me.editToolbar = Ext.create('Ext.toolbar.Toolbar', {
                ...
                items: [
                    { text: me.L('Завершить'), iconCls: 'fa fa-check', handler: me.finishRouteEditing, scope: me },
                    { text: me.L('Отмена'), iconCls: 'fa fa-times', handler: me.disableRouteEditMode, scope: me },
                    '-',
                    { text: me.L('Удалить последнюю'), iconCls: 'fa fa-undo', handler: function () { me.state.editingRoutePoints[me.state.editDirection].pop(); me.drawEditingPolyline(); me.updateEditToolbarStats(); }, scope: me },
                    { xtype: 'tbtext', text: me.L('Точек: 0') }
                ]
            });
    ...
    updateEditToolbarStats: function () {
        ...
        if (textItem) textItem.setText(me.L('Точек: ') + me.state.editingRoutePoints[me.state.editDirection].length);
    },

    enableEditMode: function () {
        var me = this;
        if (me.state.routeEditMode) { Ext.Msg.alert(me.L('Внимание'), me.L('Сначала завершите редактирование маршрута')); return; }
        ...
        me._mapClickHandler = function (e) {
            ...
            Ext.Msg.prompt(me.L('Новая остановка'), me.L('Название') + ':', function (btn, text) {
        ...
        Ext.toast({ html: me.L('Кликните по карте для добавления остановки'), align: 't', timeout: 5000 });
    },
    ...
    disableEditMode: function () {
        ...
        var mapObj = me.getPilotMap();
        if (mapObj && mapObj.map && me._mapClickHandler) {
            mapObj.map.off('click', me._mapClickHandler);
        }
    },
...
Ext.define('Store.passenger_transit.view.RouteTree', {
    ...
    initComponent: function () {
        var me = this;
        var L_fn = function(t) { return (typeof l === 'function') ? l(t) : t; };
        
        me.store = Ext.create('Ext.data.TreeStore', { root: { expanded: true, children: [] } });
        me.tbar = [
            { text: L_fn('Добавить'), iconCls: 'fa fa-plus', handler: me.onAddRoute, scope: me },
            { text: L_fn('Остановки'), iconCls: 'fa fa-edit', handler: me.onToggleEdit, scope: me },
            { text: L_fn('Рисовать'), iconCls: 'fa fa-pencil', handler: function() {
                var rec = me.getSelectionModel().getSelection()[0];
                if (rec && me.module) me.module.enableRouteEditMode(rec.data.route_id, 'forward');
                else Ext.Msg.alert(L_fn('Внимание'), L_fn('Выберите маршрут'));
            }, scope: me }
        ];
        me.columns = [
            { xtype: 'treecolumn', text: L_fn('Маршрут'), dataIndex: 'name', flex: 1 },
            { text: L_fn('ТС'), dataIndex: 'vehicle_count', width: 50, align: 'center' }
        ];
    ...
    onAddRoute: function () {
        var me = this;
        var L_fn = function(t) { return (typeof l === 'function') ? l(t) : t; };
        Ext.Msg.prompt(L_fn('Новый маршрут'), L_fn('Название') + ':', function (btn, text) {
...
Ext.define('Store.passenger_transit.view.RouteMemoPanel', {
    ...
    initComponent: function () {
        ...
        var L_fn = function(t) { return (typeof l === 'function') ? l(t) : t; };
        me.tbar = [
            { text: L_fn('Прямой'), iconCls: 'fa fa-arrow-right', handler: function () { me.showDirection('forward'); }, scope: me },
            { text: L_fn('Обратный'), iconCls: 'fa fa-arrow-left', handler: function () { me.showDirection('backward'); }, scope: me }
        ];
        me.items = [{ xtype: 'panel', itemId: 'memoContent', autoScroll: true, html: '<div class="pt-memo-empty">' + L_fn('Выберите маршрут') + '</div>' }];
        me.callParent(arguments);
    },
    ...
    renderMemo: function (route, direction) {
        var L_fn = function(t) { return (typeof l === 'function') ? l(t) : t; };
        if (!route || !route.stops || route.stops.length === 0) return '<div class="pt-memo-empty">' + L_fn('Нет остановок') + '</div>';
...
Ext.define('Store.passenger_transit.view.TimelinePanel', {
    ...
    renderChart: function (timelineData) {
        ...
        this.chart = Highcharts.chart(container, {
            ...
            title: { text: L_fn('Рейсы по времени') },
            xAxis: { categories: timelineData.hours, title: { text: L_fn('Время') } },
            yAxis: { title: { text: L_fn('Количество') }, min: 0 },
            series: [{ name: L_fn('Рейсы'), data: timelineData.trips, color: '#2563eb' }],
