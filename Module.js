...
// PILOT Extension: Пассажирские перевозки (аналог Трансфлоу)
...
Ext.define('Store.passenger_transit.Module', {
    ...
    initModule: function () {
        ...
        // Create navigation tab (Pattern A: Full UI Extension)
        ...
        // Link navigation tab and main panel (Mandatory convention)
    ...
    getVehicleTrackFromPilot: function (vehicleId, fromDate, toDate, callback) {
        Ext.Ajax.request({
            ...
            success: function (resp) {
                ...
                if (data && data.trips) {
                    callback(null, data.trips);
                } else {
                    callback(new Error('No trips data'), null);
                }
            },
            failure: function () {
                callback(new Error('API request failed'), null);
            }
        });
    },

    // ==================== ROUTE EDITOR (Polyline Drawing) ====================
    ...
    saveRoutePoints: function (routeId, points, direction) {
        ...
        Ext.Ajax.request({
            ...
            failure: function () {
                Ext.Msg.alert(l('Ошибка'), l('Ошибка соединения'));
            }
        });
    },
                            ...
                            handler: function () {
                                ...
                                if (sel.length > 0) {
                                    Ext.each(sel, function (r) {
                                        me.unbindVehicleFromRoute(route.id, r.get('vehicle_id'));
                                    });
            ...
            buttons: [
                {
                    ...
                    handler: function () {
                        ...
                        Ext.each(sel, function (r) {
                            me.bindVehicleToRoute(route.id, r.get('id'), r.get('number'));
                        });
                        // Refresh stores after binding
                        setTimeout(function() {
                            boundStore.reload();
                            pilotStore.reload();
    ...
    drawVehicleTrack: function (vehicleId, trackPoints, routeId) {
        ...
        trackPoints.forEach(function (p) {
            if (p.direction === 'forward' || !p.direction) {
                forwardPoints.push([p.lat, p.lon || p.lng]);
            } else {
                backwardPoints.push([p.lat, p.lon || p.lng]);
            }
        });
    ...
    selectRoute: function (routeId) {
        ...
        if (mainPanel && mainPanel.timelinePanel) {
            mainPanel.timelinePanel.loadRoute(routeId);
        }
    },
    ...
    selectVehicle: function (vehicleId, routeId) {
        ...
        me.getVehicleTrackFromPilot(vehicleId, fromDate, toDate, function (error, trips) {
            if (error) {
                Ext.Msg.alert(l('Ошибка'), l('Не удалось загрузить трек из PILOT API'));
                return;
            }
            ...
            Ext.each(trips, function (trip) {
                if (trip.points) {
                    Ext.each(trip.points, function (p) {
                        trackPoints.push({ lat: p.lat, lon: p.lon, time: p.time, direction: 'forward' });
                    });
                }
            });
...
Ext.define('Store.passenger_transit.view.MainPanel', {
    ...
    initComponent: function () {
        ...
        me.items = [
            ...
            {
                region: 'east',
                xtype: 'Store.passenger_transit.view.RouteMemoPanel',
                module: me.module,
                width: 320,
                split: true,
                collapsible: true,
            ...
            {
                region: 'south',
                xtype: 'Store.passenger_transit.view.TimelinePanel',
                module: me.module,
                height: 180,
                split: true,
